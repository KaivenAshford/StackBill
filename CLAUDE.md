...
StackBill 是一个面向技术用户和自托管用户的多端同步数字资产与订阅管理平台。
...
项目需要支持 PC 端和移动端，并通过同一套后端接口实现多端数据同步。
...
暂时不需要原生手机 App，第一版先做响应式 Web，保证手机浏览器体验良好。
...
PC 端和移动端共享同一个后端服务和数据库。

所有数据必须通过后端 API 读写，前端不允许单独存储核心业务数据。

移动端和 PC 端登录同一个账号后，需要看到完全一致的数据。
